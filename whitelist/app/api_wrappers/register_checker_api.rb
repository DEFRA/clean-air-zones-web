...
class RegisterCheckerApi < BaseApi
  ...
  REGISTER_JOB_URL = '/v1/whitelisting/register-csv-from-s3'
  base_uri ENV.fetch('WHITELIST_API_URL', 'localhost:3001') + REGISTER_JOB_URL

  class << self
    ##
    # Calls +/v1/whitelist/register-csv-from-s3/jobs+ endpoint with +POST+ method
    ...
    # ==== Example
    #
    #    RegisterCheckerApi.register_job('CAZ-2020-01-08-5', '98faf123-d201-48cb-8fd5-4b30c1f80918')
    #
    # ==== Result
    #
    ...
    def register_job(file_name, correlation_id)
      log_action('Registering job with file name')
    ...
    ##
    # Calls +/v1/whitelist/register-csv-from-s3/jobs/:job_uuid+ endpoint with +GET+ method
    ...
    def job_status(job_uuid, correlation_id)
      log_action("Getting job status with job uuid: #{job_uuid}")
    ...
    def job_errors(job_uuid, correlation_id)
      log_action("Getting job errors with job uuid: #{job_uuid}")
    ...
    # ==== Result
    #
    # Returns a json, eg. "{\"filename\":\"CAZ-2020-01-08-5\",\"s3Bucket\":\"test-update-server\"}".
    #
