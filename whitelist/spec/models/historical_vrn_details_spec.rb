...
describe HistoricalVrnDetails, type: :model do
  ...
  let(:vrn_history) { read_file('whitelist_info_historical_response.json')[page.to_s] }

  before { allow(WhitelistApi).to receive(:whitelist_info_historical).and_return(vrn_history) }

  describe '.pagination' do
    let(:changes) { subject.pagination }

    it 'calls `WhitelistApi.whitelist_info_historical` with proper params' do
      expect(WhitelistApi)
        .to receive(:whitelist_info_historical)
  ...
  describe '.changes_empty?' do
    it 'calls `AccountsApi.whitelist_info_historical` with proper params' do
      expect(WhitelistApi)
        .to receive(:whitelist_info_historical)
  ...
  describe '.parsed_start_date' do
    it 'returns a proper date format' do
      expect(subject.parsed_start_date).to eq('01/01/2010')
    end
  end

  describe '.parsed_end_date' do
    it 'returns a proper date format' do
      expect(subject.parsed_end_date).to eq('24/03/2020')
    end
  end
end
